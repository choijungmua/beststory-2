...
const Room = () => {
    ...
    const [newInputState, setNewInputState] = useState(true)
    // 폼 제출 처리
...
<button onClick={() => navigate(-1)}  className="w-[60px] h-[60px] flex justify-center items-center bg-white rounded-[10px] border border-tertiary">

        <img src={arrowLeft} alt="" className="w-[12px] h-[16px] " />
</button>
<div className="flex gap-[25px] flex-col w-full">
<div className="w-full text-[20px] overflow-auto text-[#323543] h-full p-[30px] bg-white rounded-[10px] border border-tertiary">
{/* 질문 1 */}
<Question onComplete={onTextComplete} questionNumber={questionNumber} />
{questionNumber > 1 && (<QuestionInput submittedValue={submittedValue}/>)}
{/* 질문2 */}
{questionNumber > 1 && (
    <>
<SecondQuestion onComplete={onTextComplete} questionText={submittedValue}/>
    </>
    )}
{questionNumber > 2 && (<SecondQuestionInput secondSubmitedValue={secondSubmitedValue}/>)}
{questionNumber > 3 && (
    <>
        ...
        <div className="flex-col">
          <p>
            <span className="font-bold mr-1">
              {submittedValue}
            </span>에 대한 {secondSubmitedValue}목록입니다.
          </p>
        </div>
      </div>
<Answer submittedValue={submittedValue} secondSubmitedValue={secondSubmitedValue}/>
...
<RoomComplete /> 
        ...
        <RoomPrompt handlePromptSubmit={handlePromptSubmit} questionNumber={questionNumber} newInputState={newInputState} />
    )}
</div>
</div>
        </section>
