...
<section className="w-[100vw] h-[calc(100vh-80px)] flex justify-center items-center bg-[#F3F3F3]">
  ...
  <input
    id="password"
    type="password"
    className={`w-full ${falseValue ? `border-ring-blue-500` : `border-error`} px-4 h-[48px] border border-[#D8DEF3] rounded-[3px] bg-white focus:outline-none focus:ring-1 focus:ring-blue-500`}
    placeholder=" "
  />
  <label
    htmlFor="password"
    className={`absolute px-2 bg-white top-0 left-3  ${falseValue ? "text-[#323543]" : "text-error"}`}
...
<div className="relative py-4 h-[48px]">
  ...
  >
    비밀번호*
        ...
        <button onClick={() => setFalseValue((value)=>(!value))} className="bg-primary text-white h-[60px] mt-[20px] w-full rounded-[3px] border bodrder-[#CCCCCC]">계정 만들기</button>
    </div>
    </div>
</section>
